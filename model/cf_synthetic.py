...
class GCNSyntheticPerturb_synthetic(nn.Module):
    ...
    """
    def __init__(self, nfeat, nhid, nout, nclass, adj, dropout, beta):
        super(GCNSyntheticPerturb_synthetic, self).__init__()
        self.adj = adj
        self.nclass = nclass
        self.beta = beta
        self.num_nodes = self.adj.shape[0]    # are edge additions included in perturbed matrix
...
class CFExplainer:
    ...
    def __init__(self, model, sub_adj, sub_feat, n_hid, dropout,
        ...
        print(sub_adj.shape)
        if self.model_type == "GCNSyntheticPerturb_synthetic":
            print("GOOD JOB")
    ...
    def train(self, epoch):
        ...
        loss_total, loss_pred, loss_graph_dist, cf_adj = self.cf_model.loss(output[self.new_idx], torch.argmax(self.y_pred_orig), y_pred_new_actual)
